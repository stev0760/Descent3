...
**Status:** Matcen 0.8.2-dev — Phase 5.4 complete. Client UI for bot match setup (Bot Settings screen in Start a New Game flow). Phase 5.2: difficulty levels (Trainee/Rookie/Hotshot/Ace/Insane) with 7 scaling parameters. Phase 5.1: config-file bot roster, ship selection, `[BOT]` name prefix, `$servercaps` telnet command. All bot commands use `$` prefix. See `BOT_MANAGEMENT.md` for design rationale.
...
| 5.4 | **Client UI for bot match setup:** "Bot Settings" button in Start a New Game screen (listen server). Per-bot roster UI: name, ship cycling, difficulty cycling. Bot count edit, default difficulty listbox. Save/load bot settings in `.mps` files. DLL API export (`MultiBotSettingsMenu` via `fp[115]`). Delayed spawn (3s) so host can manage teams. Fix: `BotAdd` init-order bug — `BotConfigureAI` was called before `Bots[].player_slot` was set, causing bots to spawn "asleep" (no thrust, no firing, no awareness) until first death+respawn. | Complete |
