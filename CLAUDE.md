...
- **`matcen-docs/NAV_CONSOLIDATION_PLAN.md`** — **the execution plan for the nav consolidation phase**: full committee census (who can seize goal/facing/thrust, with sites), the staged Step 0–5 cut order, per-toggle disposition for all 36, and the D3 physics rulings that constrain it (drag means braking is just not thrusting; bots never resist knockback). **Read this with `NAV_DESIGN_REVIEW.md` before touching nav consolidation work** — the review is the diagnosis, this is the plan.
