...
constexpr int SKEL_MAX_NODES = 64;
...
int main() {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    if (i > 0) skel_edges[0][i] |= 1ull << (i - 1);
    if (i < 3) skel_edges[0][i] |= 1ull << (i + 1);
  }
  ...
  assert(BotSkelBuildChain(&bot, 0, 1, {0.0f}, output, 64) == 4);
  ...
  assert(BotSkelBuildChain(&bot, 0, 1, {0.0f}, output, 64) == 0);
  portal_rejected[3] = false;
  for (int target_room : {0, 1}) {
    visible_node = 0;
    int n = BotSkelBuildChain(&bot, 0, target_room, {100.0f}, output, 64);
    ...
    visible_node = 3;
    n = BotSkelBuildChain(&bot, 0, target_room, {100.0f}, output, 64);
    assert(n == 0);
  }
  ...
  visible_node = 0;
  skel_edges[0][1] &= ~(1ull << 2);
  skel_edges[0][2] &= ~(1ull << 1);
  assert(BotSkelBuildChain(&bot, 0, 1, {100.0f}, output, 64) == 0);
