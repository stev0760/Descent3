...
*   **One navigator per ship** (0.9.10, in progress): navigation grew into ten-odd cooperating subsystems as game modes were added, and they sometimes compete for the same decision. 0.9.10 gave bots a travel intent that survives being interrupted, and fixed two bugs that measurement exposed. The remaining work is structural — collapsing the subsystems into a single decision point, letting the engine fly outdoor legs on campaign maps it is perfectly capable of flying, and retiring the tuning switches that only existed to paper over the overlap.
