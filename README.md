...
> **Matcen 0.9.1-dev** — Phase 7.2b Dijkstra pathfinder: three-layer reroute chain (preferred portal → one-hop scan → full Dijkstra over BOA room graph) enables bots to navigate around physically blocked portals (bunker slits, barred windows). Extensible cost overlay API for game-mode-specific routing (CTF flag paths, Entropy room capture, anti-clustering). Built on Phase 7.1 potential field steering (5-ray wall avoidance, portal attraction, wall skating) and Phase 7.2 flow field navigation (portal-directed movement, orient override, AB facing gate). Defense validated in CTF testing — bots position near flag rooms, return stolen flags, require human players to use juke maneuvers and flanking to score.
