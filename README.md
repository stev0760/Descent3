...
*   **One navigator per ship** (0.9.11, in progress): navigation grew into ten-odd cooperating subsystems as game modes were added, and they sometimes compete for the same decision. 0.9.10 gave bots a travel intent that survives being interrupted, and fixed two bugs that measurement exposed. 0.9.11 collapses the indoor half onto a single decision point: journeys a bot undertakes on its own initiative — patrolling, exploring, going to look at something — now end in arrival far more often, measured across six map pools and improved on every one, and on a single-map test flown over identical ground every round that is worth 131 → 152 captures. Two honest limits, found by a second opinion after the first write-up: trips toward the *objective* (the flag run) did not improve on the same measurements, and an earlier claim that wedged bots "nearly halved" on a second map pool turned out to be almost entirely one bot having a bad night in the first run. Still ahead — the same treatment for outdoor legs on campaign maps the engine is perfectly capable of flying, and retiring the tuning switches that only existed to paper over the overlap.
