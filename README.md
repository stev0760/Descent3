...
**0.9.1 is the stable baseline** — the Phase 11 cost-aware router is validated and bots capture flags reliably across the map pool. The next nav target is the engine-side wall-press in non-convex rooms (intermediate-waypoint injection when the next node isn't straight-line reachable), followed by routing the `!follow`/escort path through the same router so squad commands behave on complex maps. After that:
