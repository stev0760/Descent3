...
> **Matcen 0.9.1-dev** — Phase 7.6 non-convex room navigation: a flow-field line-of-sight gate keeps the portal-directed flow field in charge inside tunnels and convex rooms but defers to the engine path-follower when a wall (e.g. an in-room glass barrier) blocks the straight line to the next portal — stopping bots from pressing into geometry they could simply route around. Flag carriers now face the home flag in their home room so the afterburner score-sprint fires, and flow-field navigation is disabled in sky-exposed rooms (`RF_EXTERNAL` buildings and `RF_TOUCHES_TERRAIN` open-air rooms) where it previously steered bots into the sky barrier on canyon maps. Validated across pumphouse/MAYHEM/frenzy/bedlam/HAVOC CTF — a massive improvement in traversal, with stuck-escalations near zero outside the hardest map. Built on Phase 7.1–7.5 (potential field steering, flow field navigation, Dijkstra rerouting, tunnel-oscillation fix). CTF flag carriers whose own flag has been dropped now divert to return it home (which then lets them score) instead of parking at base waiting — fixing the double-grab stalemate where the surviving carrier could not score off a loose flag right in front of it. Flag and Hoard carriers also sprint home with sustained afterburner (skipping the indoor "silent" AB cooldown) so they no longer dawdle back to base. A carrier whose own home flag has been stolen now sprints home to stage at base (ready to score the moment a teammate returns the flag) instead of drifting back at quarter speed. **Phase 9 "flow routes, engine steers" is now the default** (`$navrouting`, default on): the flow field and Dijkstra are reserved for routing (which goal room) while the engine path-follower handles steering, eliminating the indoor barrier-transition hover where bots refused to enter a room — validated in live 4-team CTF (bots hover-locked with it off, immediately unstuck and scoring with it on). It is a global steering policy, so Hoard and Entropy benefit alongside CTF.
...
| `$navrouting on\|off` | Routing-only steering: flow/Dijkstra pick goal rooms, the engine path-follower steers (default **on**). |
