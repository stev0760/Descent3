...
**0.9.3 is the pinned stable baseline.** The Phase 11 cost-aware router plus the Phase 12 navigation stack — the pseudo-BNode skeleton (12.5b), outdoor connecting graph (12.6), and soft-hop bridge (12.7) — give bots reliable navigation to objectives across the map pool, including custom maps the engine alone can't navigate (see `matcen-docs/NAVIGATION.md` §4/§7). The known limit is **deep non-convex, multi-story custom rooms** (e.g. cavern taverns) where the portal-derived skeleton is too sparse to capture the interior or reach an arbitrary in-room target. **0.9.4 addresses this with a ground-up navigation rewrite — a volumetric grid-seeded roadmap with hierarchical routing** (design: `matcen-docs/GRID_NAV_DESIGN.md`), which also lays the spatial substrate for tactical behaviors like flanking. *Stage 1 of that rewrite is now in flight (`0.9.4-dev`, untested): a per-room grid-seeded roadmap routed with any-angle Lazy Theta\*, behind the `$gridnav` toggle, with the 0.9.3 skeleton retained as the fallback.* After that: routing the `!follow`/escort path through the same router so squad commands behave on complex maps, then:
...
*   **Custom/arbitrary-geometry nav:** The Phase 11 router (validated) + the Phase 12 in-room and outdoor go-around stack (via-points, pseudo-bnode skeleton, outdoor connecting graph, soft-hop bridge) let bots navigate maps the engine alone can't. Dead-pins are largely gone, but some **interior-divider crossings still don't complete** — in a deep non-convex, multi-story room the portal-derived skeleton is too sparse to cover the interior, so the bot drifts toward the far exit but can't always thread it (and can't reach an arbitrary in-room point). **0.9.4 rewrites the nav substrate to fix this** — a volumetric grid-seeded roadmap (`matcen-docs/GRID_NAV_DESIGN.md`). Full live status, open issues, and tried-&-reverted ledger: `matcen-docs/NAVIGATION.md` §7.0.
