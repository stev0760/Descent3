...
> **Matcen 0.9.1-dev** — Phase 7.6 non-convex room navigation: a flow-field line-of-sight gate keeps the portal-directed flow field in charge inside tunnels and convex rooms but defers to the engine path-follower when a wall (e.g. an in-room glass barrier) blocks the straight line to the next portal — stopping bots from pressing into geometry they could simply route around. Flag carriers now face the home flag in their home room so the afterburner score-sprint fires. Validated across pumphouse/MAYHEM/frenzy/bedlam/HAVOC CTF — a massive improvement in traversal, with stuck-escalations near zero outside the hardest map. Built on Phase 7.1–7.5 (potential field steering, flow field navigation, Dijkstra rerouting, tunnel-oscillation fix).
>
> **Phase 8 — outdoor/terrain navigation (in development):** Open-air canyon maps (e.g. CanyonsCTF) are built from sky-exposed `RF_TOUCHES_TERRAIN` rooms connected across open-sky *terrain regions*. Bots now steer themselves across these gaps — generating a stable heading toward the crossing-portal mouth and committing to it via a per-bot latch, with an altitude band (15-60u AGL) that holds them at terrain-relative height and prevents ceiling-pinning. A per-frame mode decision gates the band: OPEN-TERRAIN mode (crossing open sky) applies the altitude regulator; ENTRANCE-SEEK mode (heading into a mine) disables it so bots can dive into tunnel mouths. A Y-up sky-flatten axis fix and flow-field re-enable in sky-exposed rooms preceded the full steering layer. Tunable via `$terrainsteer on\|off`. See `matcen-docs/NAV_OVERHAUL_3.md`.
