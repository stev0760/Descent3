...
**0.9.4/0.9.5 (current)** — the volumetric grid-seeded roadmap (`NAVIGATION.md` **§3.5**, `bot_roadmap.cpp`) is the primary nav substrate, shipped and validated; the Phase 12 (0.9.3) stack below stays live as the `$gridnav off` fallback until Stage 4 retires it. **Phase 12 (0.9.3, stable)** — in-room + outdoor nav for custom/arbitrary geometry, layered on the Phase 11 routing-only base. The bot still only ever sets the engine's *goal*; the engine does all steering. The stack: cost-aware Dijkstra router (Phase 11) → intra-room via-point go-arounds (12.0–12.4) → **pseudo-bnode skeleton** for maps with no baked BNodes (12.5b, validated) → **outdoor connecting graph** around structures (12.6, validated net-positive) → **soft-hop bridge** across disconnected graphs (12.7, partial — ends dead-pins, not yet every crossing). `$softfollow` commitment-loosening was tried and removed (circling). **For the live toggle table + priority-ordered open issues + tried-&-reverted ledger, see `NAVIGATION.md` §7.0** (the canonical current-status snapshot). Active in CTF/objective modes — anarchy/team/robo/coop behavior-identical.
