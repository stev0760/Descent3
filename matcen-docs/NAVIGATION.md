...
| `curve` | `$curvehop` | **OFF** | 0.9.7 | **DEFAULTED OFF 2026-07-08 — isengard 16-bot A/B (soak-20260708T070322) net-negative: room-36 stucks flat/+9%, TOTAL stucks +20% (traded via-dances for wall-presses), 0 caps either arm. Likely Fork B (Theta\* straightens path[] over the mound at the GRAPH level → the hand-out has no off-chord node to walk back to; the Fork-A-vs-B discriminator was skipped before shipping — the mistake). Next: instrument path-shape, fix the straightening layer, retest at 3v3 (capture density).** Curve-following via hand-out — the chord-cutting fix (§7.0 corkscrew class). The committed via is flown as a straight `AIG_GET_TO_POS` with avoid-walls active, so the greedy furthest-visible pick (chord clearing only the bare hull 6.7) grazes the inside of a bend and the engine deflects into it (isengard room-36 left-wall press, operator POV "routed into the wall, room to spare"). Now the hand-out probe uses `BOT_VIA_HANDOUT_CLEARANCE` (13.5, ~2× hull) via new `RoadmapLOSr(radius)` so the via hugs the corridor and the bot rounds corners node-by-node; **falls back to the bare-hull furthest-visible pick when nothing clears** (tight doorways still thread). Graph build/edges stay at 6.7 — only the long committed hand-out changes. Shared by indoor + outdoor `QueryVia`. Metric: room-36 stuck-escalations/via-suspends per hour, curve-on vs curve-off. |
| `runner` | `$flagrunner` | **OFF** | 0.9.7 | **DEFAULTED OFF 2026-07-08 — A/B net-negative (CTF role, not nav; in the `$nav` namespace).** bsidectf A/B: caps/rnd 0.33 vs 0.58 legacy, grabs down, stucks MORE (myst_isle 70 vs 6). The committed runner beeline WEDGES on geometry the nav can't execute (the same chord-cutting class `$nav curve` targets), while a powerup-chasing attacker inadvertently explores its way to the flag; batteries Red reach failure persisted (0 grabs) → nav/route, not focus. Lone positive: flyable-route conversion up (batteries Blue 71→100%). Machinery: `BotAssignObjectiveLeans` fields 1 committed RUNNER (best-equipped, no powerup detours) + size/human-aware DEFEND + reactive FLEX + support ATTACK; 3v3 → 1/1/1. **Revisit after `$nav curve` validates.** `$nav runner on` re-enables live. Detail: memory `project-ctf-roles`. |
