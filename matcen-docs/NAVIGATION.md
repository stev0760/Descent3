...
| `runner` | `$flagrunner` | **ON** | 0.9.7 | **NEW 2026-07-07 (CTF role, not nav — lives in the `$nav` namespace; smoke-verified, bsidectf A/B in flight).** Dedicated flag-runner: `BotAssignObjectiveLeans` now fields **1 committed RUNNER** per team (best-equipped, routes to the enemy flag with `ctf_pushing` combat suppression but **no powerup detours** — gear-up excepted) + size/human-aware DEFEND + 1 reactive FLEX (converts to defense on a steal; runner never pulled) + support ATTACK. 3v3 → 1 runner/1 defend/1 flex; 1v1 → lone runner. Equipment sort flipped (best RUNS, was best DEFENDs). `off` = legacy binary attack/defend. Theory: soft attack-lean bots get distracted and under-convert on contested routes (batteries Red 1 grab/8rnd, pinned on the room-118 powerup trap). Detail: memory `project-ctf-roles`. |
