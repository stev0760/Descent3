...
**Status:** Matcen **0.9.11-dev (in progress, 2026-08-19)** — navigation consolidation phase, BACK HALF (`NAV_CONSOLIDATION_PLAN.md` is the plan of record; `NAV_DESIGN_REVIEW.md` is the diagnosis). Diagnosis was "fights like a pilot, travels like a committee" — the nav layer alone (routing/via/seam/hop/outdoor) drifted from the founding pilot/ship principle that combat, weapon selection, and the FSM still honor cleanly. Front half **shipped as 0.9.10 (tag `v0.9.10`, 2026-08-08)**: idle-thrust fix, goal/path lifetime, persistent travel intent with an owner hierarchy, orders outranking flag-carrying, reachability-qualified arrival. Back half to date: Task 2 (destination-churn instrument, row 6.26) and **Step 3 (one dispatch authority for interior travel legs) — code complete `74dcd573`, VALIDATED 2026-08-12 (row 6.27, §0.90) and SCOPE-NARROWED 2026-08-19 by an independent cross-model review to *explore-owned* errands (row 6.28, §0.91)**, with the 08-11 "unexplained capture gap" resolved as a cross-day measurement artifact. Remaining: cockpit session (the independent review is DONE — row 6.28), Step 4 re-run (deprioritized, gated by `tools/ab_guard.py`), Step 5 toggle-retirement audit. Prior stable: **0.9.9 (released 2026-07-19, tag v0.9.9)** — the co-op companion release (bots escort/obey orders, never autonomously play the mission; `$nav bnodesp` engine-native SP nav; phantom-pickup fix). Before that: **0.9.8 (2026-07-18, tag v0.9.8)** — the game-modes release: Entropy E1–E3 (first working bot takeovers), Monsterball M1–M2.6 (map-class scoring profile proven), CTF role structure (`$nav runner`), Hyper-Anarchy orb roles (`$nav hyper`). **0.9.7 (2026-07-12, tag v0.9.7)** — the single-spatial-authority navigation release (full history `NAVIGATION.md`, release view `CHANGELOG.md`).
