...
**Status:** Matcen 0.8.12 — CTF role auto-assignment. Team-size-aware ratios, flag-state-reactive lean switching, defender anti-bait leash, attacker retrieval redirect. Bots now self-organize into roles in CTF.

Next milestone: 0.9.0 — Hyper-Anarchy + Hoard objective behavior.
...
| 6 | **Game mode awareness + squad orders:** CTF, Hyper-Anarchy, Hoard, Entropy, Monsterball, Co-op (deferred post-launch). See game mode priority table in Phase 6 section below. | In progress (CTF complete, Hyper-Anarchy + Hoard next) |
