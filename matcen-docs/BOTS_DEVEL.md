...
**Status:** Matcen 0.9.1-dev — Phase 7.5 tunnel oscillation fix: occupancy dispersal suppressed in narrow rooms (≤2 portals) and backward-redirect guard prevents Occ-Dijkstra from sending bots back toward their origin room. Door passability check added (both sides of portal). Blind-hunt suppression (LOS required for EXPLORE→HUNT when objective active). Radius-based powerup suppression during objective nav (120u vs 350u). Built on Phase 7.4, 7.3, 7.2b, 7.2a, 7.2, 7.1.

Next milestone: 0.9.1 stable — test tunnel oscillation fix on SewerRat + abend2 CTF. Remaining known issues: 18 blocked portals on abend2 (genuine geometry, not doors), Occ-Dijkstra still uncached for rooms with 3+ portals (performance, not behavior).
