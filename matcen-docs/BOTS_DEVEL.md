...
- **Stage 2b — glass break-cost routing (`$nav glass` / `$glassroute`, default ON; added after the
  ...
  re-aligning our router with BOA, which already routes through glass. **Correction (0.9.12-dev,
  2026-08-29): that last premise was wrong, and it made this stage half-inert for two releases.**
  BOA routes through glass only *while BOA is being built* — `BOA_PassablePortal` wraps its whole
  check in `if (!BOA_f_making_boa)` (`BOA.cpp:235`), and during gameplay `BOA.cpp:244` rejects
  `PF_RENDER_FACES && !PF_RENDERED_FLYTHROUGH`, which is exactly an unbroken pane
  (`damage.cpp:1523-1524` clears that flag on both faces only when it shatters). `BotRouteDijkstra`
  tested `BOA_PassablePortal` *first*, so it threw the edge away before the finite break cost was
  ever read: the glass intent was dead code in the router. Bots still shattered panes
  opportunistically and drove through, which is why the stage looked like it worked — they just
  never *planned* through one. A router exemption for panes was built and
  measured on 2026-08-29, then **reverted** — it removed the routing failure but tripled hard stucks
  because the clearing layer did not keep up (`OBSTACLE_GEOMETRY.md` §4bb). **The gap is still
  present:** the router does not plan through glass, though bots shatter panes reactively and fly
  through, which is why the stage still appears to work. Glass-sealed rooms stop
...
| 6.17 | **0.9.8 release session (2026-07-18, from the 4-log hosted-server campaign):** operator ran an 11h metropol_gt CTF overnight + live PiccuEngine sessions (Monsterball dodgeball/PowerHouse, CTF townofbree) on a remote-hosted Linux server. Reading: 15 rounds, Red conv 30% (in-band), Blue 0% (confounded: 5-bot roster forced 3v2), stucks concentrated rooms 50/36 (wall-press class) + seam pressure rooms 55/56; townofbree = the known outdoor approach-leg class. Session fixes: (1) **`$botstat all` SIGSEGV** — the dedicated handler's private 3-entry lean table indexed with the CTF-roles-era RUNNER/FLEX leans (3/4) → garbage `%s`; killed the overnight run at the 11h mark. New bounds-guarded `BotLeanName()` is the single print path; handler also skips objnum −1 bots. (2) **Wind-blind Entropy targeting** (operator watched a RAGE bot fight a tunnel exhaust): `BotGetNearestEntropyRoom` ranked by the wind-blind BOA-chain estimate, the wind-aware router refused the pick, and `BotSetRoutedGoal`'s no-route fallback fed the engine's wind-blind path — selection now ranks by `BotComputeRouteCost` under `$nav wind`, blind-best kept as never-strand fallback; new throttled NO-ROUTE fallback line = the verification instrument (RAGE re-test pending). (3) **Log footprint**: the unthrottled carrier-objective line was 1.53M lines = ~90% of the 237MB overnight log → deduped to destination-change; footprint discipline + `$nav` telemetry-consolidation roadmap registered (NAVIGATION §5/§7.2). (4) `flag_conversion.py` counts the DLL's debris-find pickup wording (metropol_gt emits only that). (5) Repo cleanup: executed 07-07 session plan un-ingested, HANDOFF notes absorbed+ignored. **0.9.8 stamped and tagged.** | **0.9.8 RELEASED 2026-07-18** |
...
| 6.16 | **Monsterball validation day (2026-07-16, three soaks): scoring proven + the map-class profile.** Frenzy baseline: PowerHouse 3.7 / Monster Arena 3.5 goals/rnd (overwhelmingly clean bot player-goals), Veins 1.0 (the 07-15 finisher envelope alone lifted it from ~0). Fury generality (6 unfamiliar levels): HalfPipe 6.0, Taurus 4.0, TheCore 1.0 — and TheCore woke the finisher (22 slam arms; its near-dormancy elsewhere is geometry-dependent, not dead code). The profile: **arena/open + moderate corridor SCORES; extreme corridor loops (InfernalBolt, Insomniac) play-but-rarely-convert (125 fires/243 ball transitions on Insomniac, 0 goals — the ball circulates the loop); outdoor (Paranoia) is nav-bound** (striker burned the round on terrain entrance legs toward the ball's rooms; mode detection/roles/goals all correct — the Rim-analog for Monsterball). Bots play the mode on all reachable geometry; conversion depends on map class. | Mode verdict: shippable with documented map-class profile — operator sign-off pending |
| 6.15 | **Monsterball M2.6 — junction-aware ball steering (2026-07-16, operator-directed: "Veins is vanilla D3, Monsterball must generally work"):** the Veins decode (soakdump-veins.json: six 3-portal junction rooms on a loop; ~0 conversions there while open maps score) framed corridor scoring as *ball-steering at forks* — the align gate (0.80 vs the push line) can pass while the shot is still BETTER aligned with a wrong portal, and one bad nudge sends the ball a whole tube segment the wrong way. Fix: fork-argmax veto in the striker — in a ≥3-passable-portal ball room, hold the shot/slam unless `dir(bot→ball)` (exactly where a hit sends the ball) points more at the on-route portal (`BOA_GetNextRoom`) than at any other; the existing goal-side approach point repositions until the fork is won. Wired into both the fire gate and the finisher arm; fallback-safe (no route / own-goal room / <3 portals = unchanged behavior); staged as `$nav mjunction` (default on; off = the Veins baseline). Throttled `JUNCTION hold` log (+ Gametime-latch sweep for its throttle) + analyzer "Junction holds" column. **Same-day iteration + verdict:** v1 strict argmax refuted LIVE in 4 min (vetoed 0.88-vs-0.87 ties, pinned both strikers 40s+ in PowerHouse rm1 — open maps have 3+-portal ball rooms too) → v2 margin gate 0.25 (`c9bb30e6`) → **A/B VALIDATED-NEGATIVE (frenzy 6+6): goals fell on ALL maps — PowerHouse 3.7→3.0, Monster Arena 3.5→2.0, Veins 1.0→0.0** — goal-adjacent rooms ARE the hubs, so the veto suppresses exactly the finishing-band pushes (410/2892 holds were live-fire candidates); Veins had already been lifted ~0→1.0 by the 07-15 finisher envelope alone. | Default OFF — kept as `$nav mjunction` experiment lever, the gridall disposition |
...
| 6.18 | **Co-op objective play (0.9.9-dev, 2026-07-19):** the 07-19 retest flipped co-op from "broken" (stale months-old record: alleged Piccu connect failure + frozen bots) to WORKS — bots ally (`BotIsPlayerEnemy` NF_COOP), fight robots (`BotShouldTargetRobots`), respect the 3-player mission cap. This phase gives them purpose. **(1) Objective provider**: `BotPollCoop()` reads `Level_goals` active primaries (engine pre-filters + priority-sorts; the guide-bot's data source — GB logic itself lives in OSIRIS `AIGame.cpp`, we consume the same engine data directly), skips `LGF_NOT_LOC_BASED`/`LGF_GB_DOESNT_KNOW_LOC`, resolves first not-done item per LIT type (OBJECT → `ObjGet` room/pos with dead/ghost/outside rejects; INTERNAL_ROOM handle IS a roomnum → `path_pnt`; TRIGGER → `Triggers[]` face center + 5·normal via `ComputeCenterPointOnFace`; TERRAIN_CELL skipped), gates on `BotComputeRouteCost < 1e29` (locked door = not actionable — humans unlock, bots re-poll and resume). Consumed via `BotGetObjectiveRoom_Coop` (orders win: FOLLOW/COVER/position-anchor return −1). **(2) Auto-escort fallback**: no routable goal → FREELANCE bots `SQUAD_FOLLOW` the nearest human (`coop_auto_escort` flag disambiguates from chat-ordered follows; order lifecycle armed with issuer −1 so `BotOrderReport` stays silent); self-releases when a goal appears. **(3) Announcements**: `BotBroadcastAnnounce` (bot_chat) on (goal,item) CHANGE only — "Heading to: <item name>" / unreachable / all-done; engine's `GoalComplete` already announces completions. **(4) Chat orders un-gated for co-op** (`BotShouldObey` NF_COOP=true, both `Num_teams<=1` drops + team broadcast filter get NF_COOP exceptions); new `!goal`/`!objective` verb = resume autonomous objective-seeking. **(5) Quest-item safety**: `BotCanCollectPowerup` default-denies unknown powerup names in BGM_COOP via `BotIsKnownCombatPickup` whitelist (keys were never at risk — `MSAFE_OBJECT_PLAYER_KEY` never deletes the key object under GM_MULTI, multisafe.cpp:1593-1618; but generic OBJ_POWERUP quest items ARE consumed on pickup). **(6) `BotAdd` max_players clamp** (mode-agnostic; both roster and `$addbot` funnel through it). **(7) Robot-target LOS parity**: the `BotSelectTarget` robot loop picked nearest-by-raw-distance with none of the player loop's filters — through-wall matcen robots explain the observed spawn wall-aiming; now distance pre-gate (2×`BOT_FIRE_RANGE`) + `BOT_NO_LOS_TARGET_PENALTY` scoring (affects robo-anarchy too, strict improvement) + throttled `BOT FIRE` attribution line completing the glass/obstacle/main-fire triple. | In test (0.9.9-dev) — co-op campaign smoke pending |
...
| 6.19 | **Co-op smoke-2/3 batch — TRIED → FULLY REVERTED same day (0.9.9-dev, 2026-07-19):** smoke 2 (co-op-log-9.9-dev2) showed bots never leaving spawn (moth-swirl, 30+ chase timeouts, 67 occluded vias in TOROIDAL rm34 whose `path_pnt` sits in the donut hole, goal rm12 portals probe-flagged IMPASSABLE, random-looking floor/wall/clutter shots). Batch built: `BotHasFireLOS` rad-1.0 fire gate (primary fire had no wall check — true, but the gate was untested in close combat), `$nav bnode` (bnode-edge passability override + portal-bnode entry waypoints), plus two speculative loop caps (clear budget, gear-up suppression) already reverted pre-test on operator correction (spawn items are REACHABLE — the loops were nav symptoms). **Smoke 3 (co-op-log-9.9-dev3, ~30s): catastrophic — bots unable to maneuver in combat, smoked immediately by level-1 robots. Everything reverted.** Postmortem from the log: the passability override opened SIX portals in rm34 alone (plus rm32/33 clusters) — `max_rad ≥ 2.5` admits robot-scale bnode edges FAR below the Pyro hull (~6.7): **the June bnode-gen sub-hull mistake re-made with a different constant** (the ledger rule is max_rad ≥ HULL, never the fit-probe radius); the router flooded with untraversable edges and routed bots into grate slots. The fire gate compounded it (rare fire lines during same-room combat — rad-1.0 fvi sweep likely false-negatives near cluttered geometry/doorframes). Lessons re-affirmed: (1) probe-blocked portals on level 1 were mostly CORRECT (robot-only slots) — the "IMPASSABLE = bug" premise was wrong; (2) one variable per smoke — layering an untested fire gate onto an untested nav layer made the failure unreadable (the tower-of-slop pattern); (3) any future bnode consumption must gate on max_rad ≥ hull AND be soloed behind its toggle default-OFF first. | REVERTED — no code from this batch survives; co-op nav problem remains OPEN |

| 6.20 | **`$nav bnodesp` — defer to the engine's native BNode pipeline on SP maps (0.9.9-dev, 2026-07-19, `PLAN-coop-nav-rethink.md`):** 6.19's diagnosis reframed — the custom routing/via/seam/grid-route stack was short-circuiting `AIPathAllocPath`'s BNode dispatch by feeding it adjacent-hop/same-room goals instead of the far objective, so `AIGenerateBNodePath`'s multi-room walk never fired (the guide-bot's own mechanism, `scripts/AIGame.cpp:5045`, does exactly one far `AIG_GET_TO_POS`). New `Bot_bnode_native_pathing_enabled` toggle (`$nav bnodesp`, default ON — no console in client-launched co-op) with the live effective check `BotBnodeNativeActive()` (`= enabled && BNode_allocated && BNode_verified`; a function, not a level-start cache, so mid-level `$nav` flips act immediately and no BotReinitAll-timing dependency exists — review fix) — inert by construction on every BNode-less MP map. When active and both ends interior (mirrors the escort far-leg check, `bot.cpp:1794`): `BotSetRoutedGoal` bypasses `BotTrouteRedirect`/`BotComputeRoute`/seam-guard/hop-commit/`BotViaPointTick`/`$gridroute`/outdoor-entrance and issues ONE `AIG_GET_TO_POS` at the far goal (`GF_SPEED_ATTACK` only — no `GF_USE_BLINE_IF_SEES_GOAL`, no guide-bot goal-recipe mimicry per review ruling 9.6); `explore_dest_room`/`explore_room_timer` set the same way so the existing "already en route" guard holds the goal instead of churning, timer scaled by `BOA_ComputeMinDist` (not a flat 20s). `BotDoExploreRoaming`'s en-route branch skips the via re-aim under the bypass (re-issues only on lapse, same far goal); `BotApplyThrust` stuck-escape stays armed (dormant safety net, ruling 9.5.3). `BotPollCoop`'s objective-selection gate switches from `BotComputeRouteCost` to `BOA_GetNextRoom != BOA_NO_PATH` under the bypass (ruling 9.2 — selection must agree with who's flying). One-time diagnostic in `AIPathAllocPath` logs when its `f_bnode_ok` branch selects `AIGenerateBNodePath` for an `OBJ_PLAYER` goal (unfiltered, the level's robots would consume the once-latch as noise — review fix). Bot code only except that single engine-file diagnostic line. | SHIPPED 0.9.9 — validated by 3 agentic dedicated-co-op runs + operator playtest (see 6.21) |
| 6.23 | **First contention data + subtraction #1: via stands down under bnodesp (0.9.10-dev, 2026-07-22):** the 6.22 instrumentation's first real session (`testing-2026-07-22T01-33-44.log`, d3.mn3 L1 co-op, operator + 2 bots, three arms via telnet: bnodesp ON indoor / OFF indoor / ON outdoor) delivered the census the review asked for. **(1) On SP maps the "36-toggle committee" is ONE member**: across the whole session seam/hop/no-route/troute/gridroute/outdoor-entry/outdoor-leg fired ZERO times — only the via layer (+skeleton) and stuck-escape (2×) ever touched the wheel; the real fight is exactly the review's §3 top row, via-vs-engine, nothing else. **(2) The §4 indoor residual has an address**: bnodesp-ON arm showed 40 engine far-goal issues *plus 42 via seizures* — the 6.20 bypass gated `BotSetRoutedGoal`/explore but left `BotViaPointTick`'s other callers (escort close-beeline, hold-station) ungated; first NAVCONTEND flip ever logged = `via -> bnodesp after 1.0s`. **(3) Committee-OFF "basically broken" is measurable**: skeleton-via dance every 3.6s, station-keeps 22 (arm A) → **0** (arm B). **(4) Outdoor collapse is OUR gate, not the engine's**: arm C (outdoor, 202 via events across terrain cells, 0 station-keeps) ran committee-only because the bypass requires both ends interior — but the engine's own `f_bnode_ok` (aipath.cpp:1088) ACCEPTS outdoor endpoints (rejects only region-0 and cross-region out→out), and `AIGenerateBNodePath` has an explicit `BOA_connect` external-room branch; the guide-bot flies L1's canyon on the pipeline we withheld. **Changes (a+b measurement, c subtraction #1)**: (a) `NAV_MEMBER_ENGINE` — raw beeline/anchor goal issues (escort/hold, the SP-dominant legs) now count, so ours-vs-engine flips are visible; (b) `BotNavContendDumpAll()` — histogram dump+reset at A/B boundaries (any `$nav` toggle flip in `NavToggleSet`, level-end at top of `BotReinitAll`) so arms self-report to the log (the 07-22 histograms died unqueried); (c) **via stand-down gate at the top of `BotViaPointTick`**: `BotBnodeNativeActive() && !OBJECT_OUTSIDE` → drop commitment, return 0 — ONE gate covers every call site uniformly instead of patching three branches; interior-only to match the bypass; no new toggle (A/B lever = `$nav bnodesp` itself, control arm = the 07-22 log); inert on MP maps by construction. Accepted side effect: `*verdict_out` stays CLEAR under bnodesp so sealed-powerup counting is inert there (companion escorts don't collect items, 6.21). **Smoke #3 queued separately (one variable): relax the bypass's outdoor gate to the engine's own `f_bnode_ok` conditions** (allow outside endpoints, same-region, region≠0) — deleting an over-restriction, not adding a mechanism. NOT fixing arm B: committee-on-SP brokenness is evidence for the collapse, not a bug to chase. | Built (0.9.10-dev) — indoor smoke next; expected: residual closes, station-keeps ≥ arm A's 22, via wins ≈ 0 |
| 6.22 | **§7 nav-committee contention instrumentation (0.9.10-dev, 2026-07-21, `NAVIGATION.md §6.9`):** measurement-only, no behavior change — the review's own prescribed first step before any collapse-to-one-router work. New `BotNavMember` enum (bnodesp/troute/no-route/seam/hop-commit/via/gridroute/outdoor-entry/outdoor-leg/path_pnt/stuck-escape — the review's §3 committee table) + per-bot `nav_member_count[]`/`nav_last_member`/`nav_contention_count`. `BotNavMemberWin()` called at every point one of these actually seizes the goal or thrust (13 call sites across `BotSetRoutedGoal`, `BotViaPointTick`, `BotTrouteRedirect`, `BotOutdoorEntranceStage`, `BotOutdoorRouteLeg`, `BotApplyThrust`'s stuck-escape block); flags CONTENTION when the winner flips to a different member inside `BOT_NAV_CONTEND_WINDOW` (3s) of the previous one taking over — a slow clean handoff isn't contention, rapid flip-flop is (the "committee" tell). Both per-level reset points (`BotReinitAll`, `BotAdd`) covered, per the Gametime-latch class documented at 6.10. New `$nav contend [index|all]` telnet command dumps the running win histogram + contention total per bot (`BotFormatNavContend`), same shape as `$botstat`. Grounded in a real code audit before building (confirmed the review's claims: `BotSetRoutedGoal` at 248 lines chaining the members above vs. `BotSelectBestWeapon`/the FSM switch being single-owner clean — see `NAVIGATION.md §6.9` for the full diagnosis). Full rebuild clean, new log/command strings verified present in the deployed binary. | Built (0.9.10-dev) — awaiting a co-op/objective-nav session to produce the first contention data |
| 6.21 | **Co-op COMPANION MODE + the phantom-pickup fix + chill pacing (0.9.9, 2026-07-19):** operator design ruling — co-op bots are companions, not players ("bots playing the game autonomously is no fun"). (a) `BotCoopUpdateEscort()` polarity flip: default wing — every unordered bot auto-escorts the nearest human ALWAYS (goal availability no longer engages/releases escort); `BotGetObjectiveRoom_Coop` returns -1 permanently (autonomous objective nav OFF; `coop_goal_room/pos` survive for announcements + orders). (b) `!goal` repurposed from "resume autonomous seeking" to a position-anchored ORDER at the resolved objective (SQUAD_DEFEND + ORDER_ANCHOR_POSITION at `coop_goal_pos`, !hold lifecycle — vanguard post). (c) `!freelance` opt-out via new `Bots[].coop_no_escort` (consumed by any real order; reset per level + spawn) — without it auto-escort re-captured freed bots in 0.5s. (d) **Phantom-pickup fix**: `InvisiblePowerup` is the engine's invisible script-CAMERA anchor (AIGame.cpp `Obj_Create` camera pattern; multisafe's only mention is the remove-all EXEMPTION) — smoke-1 had allowlisted it as a pickup; bots chased cutscene cameras and ground at walls/the lvl-1 locked spawn door (agentic run 1: PRESS-attributed). Fix = `RT_NONE` skip in `BotFindBestPowerup` + `BotShouldInterruptForPowerup` + allowlist removal. A/B (runs 1→2): chase-fail rate 35%→10%, objective pushes 1.9→5.8/min, first goal-room push +2.5min→+21s. (e) **Chill pacing**: `BGM_COOP && state != FLEE → want_afterburner = false` (central gate; MP tuning untouched) — cures the afterburner-floor-slams (WEAK-tier indoor bursts + close-beeline vertical at floor items); run 3: presses −61%, stuck escalations 27→0. (f) Diagnostics: locked-door inventory at level start, `BOT PRESS` goal-attribution line in `BotApplyThrust`, `ARRIVED at objective` one-shot stamp. First companion playtest (co-op-log-9.9-dev6.log): instant wing-up, 5 station-keeps in 90s, broadcast `!goal` acked by both bots, clean exit. Known-open (order-reliability backlog): rm25-class tucked-geometry BLOCKED on ordered legs; geometry-blind escort station points (offset can land in a door/wall — needs fvi/roadmap nudge); escorts never collect items (role branch skips the seek — later polish). | SHIPPED 0.9.9 |
| 6.25 | **Consolidation Steps 0+1 + the first MP census (0.9.10-dev, 2026-08-04/05, `NAVIGATION.md §6.9`):** the nav phase moved from diagnosis to execution. A commissioned Fable 5 code census produced the plan of record (`a4f16a5d`): committee roster with seizure sites, per-toggle disposition for all 36, staged Steps 0-5, and the operator's two physics rulings — **D3 has real drag so braking IS just not thrusting** (no reverse thrust to stop), and **bots never resist weapon knockback** (a human can't; the Entropy v6 active park is flagged as over-reach under this rule and is explicitly NOT the template). **Step 1** = the review's headline find, an *eleventh* committee member nobody had listed: `BotApplyThrust`'s no-nav-dir fallback drove `forward = 1.0f`, and since `has_nav_dir` keys only off `movement_dir` magnitude (not goal existence), every goalless frame throttled the ship forward — manufacturing the stuck detector's own precondition (`speed<5 && applying_thrust`, where applying_thrust is just `fabsf(forward)>0.1`). Changed to zero thrust; indoor idle-press class went 4 → 0. Shipped with an as-built DEVIATION recorded rather than silently widened: the spec's "no live goal" gate is wrong because FLEE/EVADE carry no GET_TO goal yet must thrust and dodge rides `movement_dir` goalless — a goal-gated zero-thrust would be a combat regression smuggled through a nav step. **Step 0** = instrument repair, which took FOUR passes because the metric was wrong three separate ways, each caught by verification or review rather than by the numbers looking wrong: per-call units → episodes (`f4d20540`); held-until-someone-else-won → active hold (`3d927a75`); a decorative snapshot-reset flag that wiped the counters it snapshotted, plus a missing episode dormancy boundary (`1f99dc6b`); then honest buckets (`64ee2c97`) — 7-bucket leg-gate histogram (accept split interior/outdoor, each reject split reg0 vs badcell), region-0 `BOA_num_connect` made reportable, and `mdir=`/`path=` added to `BOT PRESS`. **The outdoor question from smoke #3 was ANSWERED and it is the worst case:** 889 leg evaluations gave `rej-end-region=233` dominant with `rej-cross-region=0`, detail repeated 21× as `start(out=1 reg=1 conn=3) end(out=1 reg=0)` — the bot stands in GOOD terrain (region 1, 3 BOA connections) and its DESTINATION is region 0, which neither the engine (no BNode data) nor our own lattice (navdump: outdoor graph regions 0/2/3 empty) covers. Our gate was never mis-evaluating; there is nothing behind it. Step 4 therefore becomes "extend coverage to region 0", not "pick an authority". **The MP census (the plan's owed Step 0d) then ran overnight**: 3 arms, ~8.5h, ZERO crashes; bedlam 4-team 12/12 and bsidectf 2-team 12/12 both `rc=0`. **(1) The co-op census badly understated the committee** — on MP five members are active per bot per round (seam/hop-commit/via/gridroute/path_pnt, e.g. `episodes(215) ... contention=91`) where co-op saw seam/hop-commit/gridroute fire ZERO times all session; §1's inferred "load-bearing on MP" column is now measured and Step 3 is worth more than co-op implied. **(2) No scoring regression** — Polaris 11.0 caps/rnd vs 8.0 in the last comparable 4-team run (0.9.3 peak 15.6, Blue conv 59% inside the 56-69% gold band), Apparition 7.0 vs 7.9 par, Plutonium 6.3 vs 4.0, QuadSomniac 4.7 vs ~0 norm; Plutonium Red 4% and bside mysterious_isle Red 16-picks/0-caps reproduce known-open items unchanged. **(3) The new PRESS discriminator indicts GOAL LIFETIME on its first outing:** 60 of bedlam's 66 goalless presses (91%) are `path>0` — bots flying an engine path that outlived its goal. Added to Step 2 as a concrete sub-item (flush the path when the goal dies; residual `path>0` goalless presses = that step's pass metric). **(4) Outdoor stucks are an entrance-approach failure** (Polaris 19/20, Apparition 18/20 "routed into a structure"), the region-0 coverage boundary seen from the indoor side — so Step 4's seeding must treat structure-entrance stitching as first-class. **(5) Entropy arm (6/6, 0 crashes) REPLICATES the goal-lifetime finding on a third mode: 93 of 102 goalless presses (91%) are `path>0` — the identical ratio to bedlam's 60/66**, making the Step 2 path-flush the best-evidenced single change in the plan; also showed the committee shape is MODE-DEPENDENT (Entropy is via-monopolised, 489-780 via episodes/round vs CTF's ~87, with seam/hop/gridroute near zero) and contention is wildly per-bot within one round (Ninja 8, Gregg 16, Viper 574) — situational/geometric, not load-driven. Worst bot-round (Viper 295 stuck-escape episodes) was on Rim, the documented nav-hostile map under a standing ignore-Rim directive = NOT a regression. **(6) STEP 2a SHIPPED + VALIDATED 2026-08-05 (`d9ba49d6`): "no live goal => no live path".** Root cause = `GoalClearGoal` frees the engine path ONLY when `path.goal_uid` matches (AIGoal.cpp:567), so any slot overwrite orphans a live path that keeps feeding `movement_dir`. First attempt (free inside `BotClearActiveGoal`) DID NOTHING — a 7-min smoke still showed 4/4 goalless presses with `path>0`, because ~25 `GoalAddGoal`/~20 `GoalClearGoal` sites sit outside it, each re-issue clearing its own slot through the same uid-gated free. Fix that worked = an INVARIANT enforced once per bot per frame (`BotEnforceNoOrphanPath`), not 20 call-site patches. **A/B vs the census (configs byte-identical, rounds cut): stale-path share 91%→8% on bedlam and 91%→12% on Entropy = a 90% per-round reduction on BOTH modes independently, 0 crashes.** Total goalless presses barely moved on bedlam (5.5→6.0/rnd) — presses converted from stale-path into the residual dodge class rather than being suppressed, exactly as predicted since dodge residual is combat-layer/out-of-scope. **(7) STEP 2b-1 SHIPPED (`c0f04978`): human orders now OUTRANK carrying the flag** — operator ruling; previously `BotIsCarryingEnemyFlag` was the dispatcher's FIRST branch so a carrier under `!follow`/`!hold` never even evaluated the order. Safety traced first: nothing auto-assigns `squad_role` in MP (the only auto-assigner, `BotCoopUpdateEscort`, is reachable only under `BGM_COOP`, which has no flags/orbs/virus loads). No-op invariant test (4 rnd, no orders issued) matched the 2a build on every axis. **(8) NIGHT-2 FULL CENSUS (identical manifests, only the build differs) — 2a HOLDS AT VERDICT LENGTH AND EXPOSED A LATENT DEFECT.** Stale-path share bedlam 91%→9%, bsidectf 46%→6% (the untested outlier, fell hardest), Entropy 91%→3%; 0 crashes on all three; **bedlam captures 94→114 (+21%)** with carrier deaths down on every map (Polaris the lone exception, 11.0→9.3 caps/rnd). **BUT hard stucks 4→52 on bedlam** (Plutonium 2→31, Polaris 2→18) and stuck escalations/rnd bedlam 2.0→9.4, Entropy 5.2→32.7. **Mechanism: the orphaned path was ACCIDENTALLY LOAD-BEARING** — it kept goalless bots moving (badly, but moving); removed, a goalless bot has only reactive steering (all 96 bedlam goalless presses log `mdir=1.00 path=0` — dodge/wall-avoid with no destination to pull it out). This is the review's VACANCY pole and the "chaos provides accidental robustness" risk, landing as predicted. **The exception is the tell: bsidectf (tight indoor) shows NO increase (21.1→19.8) while outdoor-heavy bedlam/Entropy show 4.7x/6.3x** — a goalless bot in a corridor gets steered along it and re-acquires a goal; in open space it has nothing. **Consequence: Step 2b moves from "next" to REQUIRED — 2a shipped the subtractive half of one change, and no further subtraction should ship until persistent intent supplies the replacement motion.** **(9) 2b-2/2b-3 (intent survives state flips + powerup detours) shipped, and night 3 REGRESSED** — bedlam escalations 9.4→12.0/rnd, captures 114→86, bside 19.8→26.6, `mysterious_isle` to ZERO caps; rooms-per-escape fell 0.245→0.171 = the stubbornness pole. **(10) ROOT CAUSE was NOT the 2b pair — it was 2a resting on a FALSE premise (`31873fd1`).** `BotConfigureAI` installs a permanent engine `AIG_WANDER_AROUND` goal (priority 1, `GF_NONFLUSHABLE` + `GF_KEEP_AT_COMPLETION`) that legitimately allocates paths whenever no level-2 goal is live — 2a was freeing them EVERY FRAME, so the engine re-rolled forever: "Wander is generating the same room" 64,129→122,982→191,131 across nights 1-3. **A TWELFTH committee member, invisible to every instrument**: wander never passes through `BotNavMemberWin`, and PRESS reported it as `goal=none`. Product = a ghost class (`goal=none path=0 mdir=1.00` at 3-5 u/s — freeing a path never cleared `movement_dir`, so the ship crept along a dead vector below the stuck threshold but above Step 1's `has_nav_dir` gate): 6→87→81 presses. Fix = free only when NO used goal in `goals[]` claims the `goal_uid` (the engine's own contract, AIGoal.cpp:567, all ten slots not our three) + zero `movement_dir`; plus the fifth lifetime cause (demote a destination that forced an escape for 45s AND mark it visited — the explore scorer's +100 unvisited bonus actively preferred the room that had just beaten the bot). **VALIDATED 12 rounds: ALL FIVE pre-registered gates passed and the fix build is the BEST of the series** — wander 64,959, ghost 3, rooms/escape 0.667, escalations 1.0/rnd, **captures 121** (N1 94 / N2 114 / N3 86); stucks/rnd below N1 on every map. Conversion (duration-independent, finally run on all four nights): Apparition 24%→45%, Plutonium 16%→23%, QuadSomniac 6%→9%. **2a's original 91%→9% headline was INFLATED BY FRIENDLY FIRE** (much of N1's "stale" was wander's legitimate path) and **the stale-path SHARE is retired as a metric** — post-fix it reads 94% while the absolute count is lower than N1's. **OPEN: Polaris regressed** (conv 35%→30%, 11.0→8.7 caps/rnd; Blue 39 picks/11 caps vs N1 22/13 = more grabs fewer scores = RETURN-LEG failure, the wind-tunnel map). **Process lesson: three consecutive one-variable-rule violations (N2 bundled 2a+2b1, N3 bundled 2b2+2b3, and a 4-round A/B trusted for a metric it could not resolve) — bundle only when each change has a pre-registered metric the other cannot move, and 12 rounds minimum for anything gated on escalations or captures.** | Steps 0+1+2a+2b-1+2b-2+2b-3 SHIPPED and VALIDATED at 12 rounds (`31873fd1`, best-of-series: 121 caps, escalations 1.0/rnd). **RELEASED AS 0.9.10 (tag `v0.9.10`, 2026-08-08)** — the phase's front half stamped stable deliberately, to leave a tagged anchor before Step 3 (a behavior-neutral refactor of the highest-traffic path in a 396 KB file, i.e. the exact shape that produced the reverted 05-30 batch). **The back half runs under 0.9.11-dev:** Polaris forensics → the destination-churn instrument built as Step 3's dispatch seam → Step 3 (single dispatch, one call site per commit) → Step 4 (SP outdoor gate deletion, probe-confirmed) → Step 5 (toggle retirement audit). OPEN = Polaris return-leg regression; 2b-1 order>carry still needs a human cockpit test |
| 6.26 | **0.9.11-dev back half — Task 2 (churn instrument) + Step 3 (one dispatch point) (2026-08-09/11, `NAVIGATION.md §6.9`):** the phase's back half. **Task 2 CLOSED** (`58ddbb9c`): the census found `explore_dest_room` doing TWO jobs (errand intent AND routed per-waypoint bookkeeping), so intent went to SHADOW STATE (`travel_dest_room`/`travel_owner`/`travel_set_time`) written only by a typed setter — every legacy write byte-identical, behavior-neutral by construction. `BotSetRoutedGoal` gained a `BotTravelOwner` parameter = Step 3's dispatch seam cut early; `BOT DEST` lines carry owner + the five lifetime causes; analyzer gained a Travel Intent section + `DEST_CHURN`. 4-round gate passed. **Step 3 = 6 commits, interior legs only** (`7f8b1d3e` escape retarget, `ed6ea2eb` en-route maintenance = the substrate shift, `6102ce36` last-target, `675fe909` random explore, `8490e111`+`74dcd573` the two corrections): outdoor untouched (operator guardrail), powerup chase + escort pair deliberately deferred (escort is cockpit-only). **FOUR 12-round arms on identical manifests, zero crashes, and the churn instrument earned its existence on its first outing:** arm 1 = the AMNESIA pole (errand re-rolled at every hop — events 2x, median life halved, explore re-picks 1122->2458) and was **capture-NEUTRAL (113 vs 114)**, i.e. a capture-gated verdict would have PASSED a build that silently broke the layer this phase exists for; arm 2 fixed it and hit the STUBBORN pole (objective `replacement` ends 530->18, objective nav re-issues 3667->866 — an objective errand held to arrival is a trip to where the flag WAS), churn ideal but **captures 114->100**; arm 3 (`74dcd573`) restored objective re-evaluation (`replacement` 1084 ~ control 1003) with the churn shape back in family, and **GUARD-FAILED on outlier dominance** (Zed = 74% of the escalation delta, 19/21 in Apparition room 0, median net_disp 16 = CIRCLING not pinned; new registered signature). **Step 3 verdict: STRUCTURALLY LANDED, NOT VALIDATED** — captures sit 98-100 vs 114 with NO measured mechanism (arm 3 disproved staleness as the cause), while carrier death DISTANCE moved closer to home (Polaris 517u->471u, Apparition 586u->502u) with deaths rising, which under matcen-triage's own rule reads as INTERCEPTION/defence, not return-nav failure. Operator ruling: captures are one variable among many, a 12% dip with more combat may be BETTER play — **no further code changes; cockpit session + independent review are the next instruments.** Also shipped: `ab_guard` INTEGRATED into `soakctl` teardown (an external audit correctly called the remember-to-run-it version a live process defect) — manifests carry control/pin/expected-rounds, the driver stamps `guard=PASS|FAIL` on `SOAK_DONE`, and it FAILED correctly on its first live arm. | Step 3 code complete + documented; **validation CLOSED 08-12 — see 6.27** |
| 6.27 | **Step 3 VALIDATED — and the "unexplained" capture gap was a cross-day measurement artifact (0.9.11-dev, 2026-08-11/12, `NAVIGATION.md §6.9`):** four more paired data sets under the standing no-code-changes ruling; build `74dcd573` vs control `58ddbb9c` throughout, each arm deploying its own hash-stamped binary, `ab_guard` at every teardown, **zero crashes in all eight arms**. **(1) THE ERROR, found by re-measuring the control against itself:** 6.26's control ran 08-10 and its arms 2/3 ran 08-11 — bedlam Polaris conversion on the *same binary* reads **47% (08-10, picks 91) vs 37% (08-11, picks 90)**, i.e. **10.6 points of swing with zero code change.** The 114→98 capture gap and the "Polaris regression" were both read across that boundary. A same-evening replication A/B (12 rnd each, only the build differing, both guard-PASS) puts conversion **flat on every map** — Apparition 34/36%, Plutonium 26/24%, QuadSomniac 7/8%, **Polaris 36/35%** — captures 114 vs 110, kills 129 vs 131. **Third instance of this class** (Step 4's broken level pin, the 08-08 Polaris withdrawal, this one) → **standing rule: arms run back-to-back on the same machine the same evening or the comparison is not made.** `ab_guard` proves structural comparability and says NOTHING about when the arms were measured — a named gap in the harness, not a fixed one. **(2) VALIDATION ON THE LAYER STEP 3 ACTUALLY CHANGED**, the Task 2 churn instrument (hundreds-to-thousands of errand lifetimes per round vs 3-4 scoring events), four independent pools, arrival share / timeout share of finished intents: bsidectf 7.3%→**12.4%** / 41.0%→**9.3%**; Fellowship 8.7%→**13.2%** / 19.8%→**10.4%**; KegD3 17.0%→**19.9%** / 14.2%→**12.6%**; bedlam Polaris 18.0%→**22.5%** / 14.0%→**9.7%**. `DEST_CHURN` fires on **3 of 3** bsidectf maps on control and **0 of 3** on Step 3; median errand life rose on 8 of Fellowship's 9 maps. **(3) SCORING, where the surface is clean enough to read it:** KegD3 3v3 (single level = identical geometry every round, 13 rnd each) **captures 131→152 (+16%), stucks 163→87, hard 29→16**, powerup pins 109(17)→66(13), carrier death distance flat (222u→224u); bsidectf (12 rnd vs the identical 08-10 manifest) **hard pins 62→33**, kills 107→113. Both guard-FAILED on outlier dominance (Hawk 53%, Reaper 47%) and **both survive exclusion** (112 vs 76; 244 vs 212) — the opposite of arm 3, where exclusion reversed the direction; the guard forces the segmented read rather than vetoing the result. **(4) TWO BOOKKEEPING CAVEATS PUBLISHED WITH THE CLAIM:** Step 3 moved the sites that *record* errand ends, so `unreach` is **not** cross-build comparable (unconditional `BotClearTravelDest` at `bot.cpp:6145` vs a room+owner-deduped `BotSetTravelDest` at `58ddbb9c:6074`) and no claim rests on it; the two classes that DO carry claims are biased *against* the finding (HEAD over-counts TIMEOUT vs control at `bot.cpp:3490`/`58ddbb9c:3438`; #5 made the arrival test stricter), so both reads are conservative. **(5) TWO ERRORS RECORDED RATHER THAN BURIED:** the Fellowship arm (9 rounds over 9 maps, 1 round each — `gollumspursuit` alone supplied 4 of the control's 12 captures) is too thin to read as a build comparison and is not treated as one, though its structural signal is real (`towerofisengard` stucks 20→6/rnd, a registered problem map); and the Entropy arm hard-stopped at 3 of 6 rounds on `max_minutes: 120` when its own 08-10 control took 180 — manifest raised to 240, arm still **owed**, and with it the re-read of the Wishbone/Inversion `DEST_CHURN` flags. **(6) New registered signature: Plutonium room 2** — all 7 of the replication arm's hard pins in one room (corrects the first pass, which reported it as Apparition room 2). **(7) ENTROPY ARM RUN 2026-08-13 as a proper same-evening pair** (3 rnd each = one clean CHAOS pass, identical level sequences, 0 crashes) — **the largest churn movement on any pool**, on the most timeout-dominated mode measured: arrival share 6.8%→**16.5%**, timeout share **54.3%→21.1%**, Wishbone arrival 11.2%→**35.9%** with median errand life 21.5s→**44.6s** and `DEST_CHURN` **cleared**; `ENTROPY_ZERO_TAKEOVERS` on Wishbone reproduces UNCHANGED on both builds (pre-existing mode item, not dispatch's). Arm is GUARD_FAIL (`Reaper` = 70 of 85 escalations, 97% of the delta; excluding it 13 vs 15 = flat), so its escalation totals are not a population result. **(8) THE SEGMENTED READ FOUND MORE THAN THE ARM — the ESCAPE-RELAPSE LOOP.** Reaper livelocks: stuck Inversion rm35 → escape to rm6 → errand `none->6` → `6->none end=unreach` after 12-15s → stuck → escape to rm6 again; rms 35 and 7 both drain into rm6, **53 escapes to the same never-reached room**. **The identical shape is in every arm on BOTH builds** (bedlam control rm15→rm0 x4; bedlam Step 3 **rm2→rm1 x7** = the "Plutonium room 2" cluster; arm 3 **rm28→rm27 x23 / rm0→rm84 x20** = the "Apparition room 0" cluster; bsidectf rm32→rm33 x27 control / x54 Step 3). **Mechanism from code:** the escape portal chooser (`bot.cpp:6113-6131`) filters bounds/`PF_TOO_SMALL_FOR_ROBOT`/passability, skips `croom == explore_dest_room`, and only *prefers* unvisited — **it never consults `failed_dest_room`**, the fifth-lifetime-cause blacklist from `31873fd1` whose ONLY consumer is the explore scorer (`bot.cpp:3389`); and since `BotRecordVisitedRoom` records only rooms actually ENTERED, a never-reached target stays "unvisited" forever and keeps winning the preference (arm 3's 43 dominant relapses are all labelled `unvisited` after dozens of failures). **NOT a Step 3 regression — present on every control arm.** Three separately-registered signatures are very likely ONE build-independent defect, outside this phase. Fix class: escape chooser consults the blacklist, ideally a short ring of failed escape targets rather than one slot; ready-made A/B = escapes-per-bot concentration + the relapse-pair histogram. | **Step 3 VALIDATED** (§0.90 revises §0.88); all five pools now read. Machine left on `74dcd573`, md5-verified. Still open: cockpit session + independent review, **the escape-relapse loop** (registered, not fixed — no-code-changes ruling stands), and outdoor scoring variance → the region-0 coverage campaign, not dispatch |
| 6.28 | **INDEPENDENT CROSS-MODEL REVIEW — Step 3 survives, the claim narrows (0.9.11-dev, 2026-08-19, `NAVIGATION.md §6.9`; §0.90 corrected inline):** the operator commissioned a fresh-eyes review of 6.27 from a different model family (GPT-5.6 Sol, Opencode; transcript `session-ses_fe83.txt`) precisely because §0.88-0.90 were written by the model that wrote the code. Review-only. **Every finding was re-verified against source and re-computed from the logs before acceptance** — and the re-computation mattered: a first attempt to reproduce the owner stratification FAILED until the `BOT DEST` attribution rule was read off the log format properly (a supersession line carries the ENDING intent's owner in `prev=`, not `owner=`; a naive `owner=`-first regex silently produces a plausible, wrong table). **Verdict: the code stays landed; three measurement defects and one wrong mechanism claim.** **(1) `unreach` WAS in the published denominators** — `analyze_bot_log.py:668` computes `finished = sum(de.values())`, so §0.90's own "no claim rests on `unreach`" was false of every arrival/timeout SHARE it printed. **(2) TIMEOUT SHARE IS CLOCK-CONFOUNDED IN STEP 3's FAVOUR — the §0.90 caveat had the bias backwards.** Control gives one explore errand a single distance-scaled 6-20s timer (`58ddbb9c:bot.cpp:3436-3451`); Step 3 routes explore through `BotSetRoutedGoal`, which sets `explore_room_timer = BOT_EXPLORE_ROOM_TIME_MAX` (the full 20s) **on every hop** (`bot.cpp:3083`) while the intent stays continuous — one Step 3 intent collects several timeout windows where control's errand got one. **(3) ARRIVAL WAS NEVER MADE STRICTER THAN CONTROL** — `8490e111` made it stricter than the broken first arm and restored control-equivalent semantics (`bot.cpp:317-360` identical in both builds); "arrivals rose against a raised bar" was wrong. **(4) THE FINDING THAT CHANGES THE STORY — the pooled table was averaging two populations moving in OPPOSITE directions.** Recomputed excluding `unreach` and stratified by the ending intent's owner, across all six pools: **explore-owned arrival rises in every one** (bsidectf 11.0→30.8%, replication 13.5→29.7%, Fellowship 11.8→32.5%, KegD3 22.7→34.3%, Polaris 26.8→45.3%, Entropy 6.9→26.5%) while **objective-owned arrival FALLS in every one** (10.1→9.3, 13.0→7.2, 9.1→6.3, 16.8→13.0, 12.1→10.7, 15.5→6.8). Partly by design (Step 3 #6 makes objective errands re-evaluate, converting would-be arrivals into `replacement`s, so `arrival` means something different for that owner after #6) — but objective-owned TIMEOUT also rose where sample is largest (KegD3 233→342, Polaris 61→81), which re-evaluation does not explain. Counterweight against over-reading it: KegD3 captures rose 131→152 in the same arm. **(5) SAME-EVENING bsidectf REPLICATION** (12x15min rounds/arm, hash-verified binaries, identical level sequences, 0 crashes; `soak-20260818T233102.log` vs `soak-20260819T023142.log`) — 6.27's bsidectf arm was control 08-10 vs test 08-12, i.e. **unpaired, in the section that made pairing a standing rule.** Intent result replicates hard (13.2→25.1% arrival, surviving outlier exclusion at 14.2→27.3%); indoor maps carry it (batteriesincluded 11.3→31.5%, Nightmarecastle 14.8→34.7%) and **`mysterious_isle`, the outdoor-connected map, did NOT improve** (11.8→9.8%) — consistent with Step 3 owning interior legs only. **(6) THE `62 → 33` HARD-PIN HEADLINE IS ONE BOT.** Replication reproduced the raw drop almost exactly (63→34) and segmentation killed it as a population claim: `Gregg` alone 26→1, everyone else **37→33, flat**. The clearest demonstration yet of why `ab_guard` exists — and it cuts AGAINST a Step 3 claim. KegD3's 29→16 has never been segmented and is now treated as unconfirmed. **(7) `DEST_CHURN` "3/3 → 0/3" OVERSTATED A THRESHOLD CROSSING** — the ratio is the evidence: batteriesincluded 7.37→1.75, Nightmarecastle 4.96→0.89, `mysterious_isle` **3.62→3.62** (identical, already under the 4.0 alarm on both builds); paired alarm count is 2/3→0/3. **(8) THE ESCAPE-RELAPSE MECHANISM IN 6.27 WAS WRONG, AND ITS FIX CLASS WOULD LIKELY NOT HAVE WORKED.** Verified at HEAD: the chooser DOES skip the live destination (`bot.cpp:6121-6123`) and the escape DOES write failure memory INCLUDING `BotRecordVisitedRoom(esc_dest)` for a never-entered room (`bot.cpp:6183-6186`) — both halves of the published mechanism are false. **The real hole is the room-progress timeout** (`bot.cpp:8351-8353`): the non-escalation branch clears `explore_dest_room` and the intent while writing NO `failed_dest_room` and NO visited mark, so the room keeps the explore scorer's unvisited bonus and can win again — and by the time an escape fires, `esc_dest` is already -1, so the `esc_dest >= 0` gate skips the blacklist write entirely. `failed_dest_room` has exactly ONE writer and ONE reader (`bot.cpp:6184` / `bot.cpp:3389`), grep-verified. Second defect in the same area: since Step 3 `explore_dest_room` holds the routed WAYPOINT while the explore filter tests `failed_dest_room` against sampled FINAL destinations — the blacklist and the filter are not naming the same room. **Fix class restated: decide who owns failure memory and make the TIMEOUT path write it; do NOT just add a chooser blacklist read.** **(9) NEW OPEN SIGNATURE — seam refires:** exact same-bot/same-detour/same-waypoint retries at the 5s latch cadence, Nightmarecastle **0 → 232** (batteriesincluded 2→37, `mysterious_isle` 53→46); via suspensions flat (~+2%). Not a latch bypass — an unresolved hop retrying indefinitely UNDER the throttle. **(10) INSTRUMENT DEFECTS:** `ab_guard.py:92` compares only the LENGTH of the two level sequences, not the sequences, and prints reset counts without failing on a mismatch (arms here were manually verified identical — nothing corrupted, but the guard is weaker than the places citing it as proof imply); `bot.h` still claimed nothing reads travel intent at runtime, false since Step 3 reads it at `bot.cpp:3121-3154` (comment corrected — comment-only, no behaviour change). **(11) THE CROSS-DAY FRAMING WAS TOO STRONG:** the same-binary swing proves run-to-run VARIANCE, not a proven date effect (same-build cross-day -10.6 pts, ~95% CI -24.9 to +3.7; same-evening A/B -0.9 pts, CI -15.3 to +13.6 — both contain zero and each other). Correct claim = "the regression did not reproduce", not "proven artifact" and not "builds proven equivalent". **Order is still confounded: every clean pair ran control first.** **(12) SURVIVED UNCHANGED:** the 08-10/08-11 same-build swing, the same-evening bedlam conversion table, all 7 hard pins being Plutonium room 2, the relapse SIGNATURE on both builds, intent direction surviving outlier removal in all four flagged pools, and zero crash/stale-binary/rule-bending findings. | **Step 3 stays landed, scope = explore-owned errands.** §0.90 corrected inline + §0.91 added; CHANGELOG/README overclaims corrected; the review brief marked done. Standing rules added: exclude `unreach`, stratify by owner, segment the PRIMARY metric by bot (not just the guard's), reverse arm order in one pair, and stop quoting timeout share as an effect size. Open: objective-owned degradation, Nightmarecastle seam refires, the relapse loop (fix class restated), `ab_guard` sequence check. Next instrument = the operator's cockpit session |
| 6.24 | **Subtraction #2 (outdoor gate = the engine's own `f_bnode_ok`) + smoke #3 — SPLIT VERDICT: indoors won, outdoors did not move (0.9.10-dev, built 2026-07-24 `769d6b3a`, run 2026-08-04):** the 6.23-queued one-variable smoke finally ran. **Build:** `BotBnodeLegOk(start,end)` mirrors the engine's gate verbatim (aipath.cpp:1087-1092 — accepts outdoor endpoints, rejects only terrain region 0 and cross-region outdoor→outdoor) and replaces BOTH both-ends-interior checks (the `BotSetRoutedGoal` bypass and the 6.23 via stand-down); `BotTerrainRegionSafe()` bounds-guards `TERRAIN_REGION` against the -1 sentinel (it indexes `Terrain_seg[]` unchecked); `BotRemoveAll()` gained a contend-histogram dump. Per-leg by design: a leg the engine declines keeps the full via layer as fallback. **Smoke #3** (`smoke3-2026-08-04T18-43-24.log`, d3.mn3 L1 co-op, 2 bots, operator joined 18:44:07, ~21 min, build `9ed03398`; cfg = `soak-smoke3.cfg`+`soak-smoke3-bots.cfg`, a clone of `testing.cfg` — the 07-22/07-23 control-arm config — differing only in GameName and console password, so PPS=40/TimeLimit=30/difficulty/powerup-disallows all match). Live histograms: Reaper wins(434) bnodesp=212 via=123 engine=99 contention=**16**; Phantom wins(1026) bnodesp=126 via=270 **stuck-escape=509** engine=121 contention=**181**. **(1) INDOORS = VALIDATED: zero indoor via seizures**; all 42 `bnodesp far goal` issues went to interior rooms; bots reached deeper into L1 than ever recorded (34→31→28→12→46→86) and tripped the first objective — operator: "MUCH farther than I've seen," and independently "bots feel a good bit more coherent." **(2) OUTDOORS = UNCHANGED: the engine received ZERO outdoor legs (0 of 42)**; 456 of 463 skeleton-via events are outdoor (negative `ROOMNUM_OUTSIDE` rooms), none found indoor — i.e. `BotBnodeLegOk` returned false for EVERY outdoor leg on this map, so the relaxation never actually fired. Operator matched it from the cockpit: "couldn't get them to lock in on an objective when outside." **(3) UNVERIFIED REFRAME (do not treat as finding):** if those legs fail on region grounds, the ENGINE declines them too (same conditions) → the engine is **not an available authority outdoors**, and outdoor consolidation must ride OUR 0.9.4 roadmap instead — which the navdump shows is already healthy there (outdoor roadmap region 1: 1044 nodes, 1 comp, non-degenerate; outdoor graph region 1: 15 nodes / 3 entrances, regions 0/2/3 empty). That would invert subtraction #2's premise from "our gate is too strict" to "there is nothing behind the gate." **(4) The "erratic/ADHD" feel now has a measured signature:** Phantom's stuck-escape won 509× (≈50% of its wins) while `analyze_bot_log.py` reports **0 hard stucks** — escape thrust seizing the wheel on a bot that is not pinned; Reaper contention 16 vs Phantom 181, same map and minutes. Both bots ended `state=EXPLORE dest_room=-1 route:goal=-1` (no destination at all). **(5) Follow reliability regressed and is measurable:** 17 on-station arrivals but only 5 escort station-keeps, ALL Phantom (**Reaper 0**), vs ~30 in the shorter 07-23 arm; 2 blocked reports; 11 `BOT PRESS` (several `goal=none` at spd=0; outdoor ones steering at targets 1971-2327u away); analyzer also 438 skeleton hops, 16 VIA_SEARCH_FAIL (room -1 ×15), 24 grate clears. **(6) TWO INSTRUMENTATION GAPS — the next action is (a), not more code:** (a) `BotBnodeLegOk` logs NOTHING about which of its three reject branches fired, so the §(3) hypothesis cannot be settled from a log — a 3-way reject counter (~10 lines) converts the outdoor question from argument into a number; (b) **SIGTERM does not run `BotRemoveAll`**, so this row's own histogram-dump fix does not cover the ACTUAL shutdown path (`$quit` over telnet re-confirmed ignored) — capture `$nav contend all` over telnet BEFORE killing the server or the run's numbers are lost, as they nearly were here. Companion mode is NOT regressed: the goal trace is explore-roaming (`BotGetObjectiveRoom_Coop` still -1), so the switch trip was incidental; and the bots had only 42s alone before the operator joined, so the deep progression happened with him in-game. | Run 2026-08-04 — indoors SHIPPED-quality, outdoors NO-OP. Operator ruling: **stay in triage, no big code yet**; gap (a) instrument first, then decide whether outdoor authority is the engine or our roadmap |

### 6.29 Step 3 cockpit exit, Step 4 closure, Step 5 tranche 1 (0.9.11-dev, 2026-08-22)

The KegD3 CTF cockpit session completed Step 3's owed feel gate. Operator verdict after the full
round: **"Feels excellent."** Owner-correct decode (`prev=` on supersession, otherwise `owner=`;
`unreach` excluded) put explore arrival at 44%, with both teams scoring and only KegD3 room 32
producing hard pins. Step 3 is closed for explore-owned interior errands.

The session also exposed Task 2's exact telemetry boundary: a CTF capture does not explicitly end a
travel intent, so 5 of 15 bot capture events left successful carry intents live until a later death.
Carry/objective arrival and pooled `DEST_CHURN` are therefore not success measures. The analyzer now
stratifies outcomes by the owner of the ending intent, excludes `unreach` from completion shares, and
gates churn on explore-owned errands. Both CTF parsers now recognize two- and three-flag cash-ins,
which historical four-team conversion silently omitted.

Step 4's gate widening is **closed-no-go**, not queued for another long A/B. The existing reclaim
probe measured 356 blocked vs 3 clear (99.2%), so `legacy_accept || BOA-routable` would send almost
the entire class to the engine's unproven coarse outdoor fallback. The old SP gate and outdoor
substrate remain; region-0 coverage stays a separate post-consolidation build.

Step 5 tranche 1 removes the three default-off mechanisms already dispositioned as dead:
`gridall`, `outroute`, and `replan`. Their flags, command rows, stall state, and obsolete enabling
manifests are gone (36 to 33 `$nav` rows). Default behavior is unchanged; default-ON `troute` still
owns `BotOutdoorRouteLeg`, and historical analyzer parsers remain. `ab_guard.py` now compares exact
level sequences and reset counts; `soakctl.py` fails immediately if a requested toggle is unknown.
