...
- **Bots ignore player cloaking** — When a player picks up the cloaking device, bots continue to track and engage them as normal. Bots should mirror vanilla D3 robot behavior: a cloaked player is undetectable unless they use their afterburner, fire a weapon, or collect a powerup. Implementation will require hooking into the existing cloak state checks (`EF_CLOAKED`/`EF_FADING_OUT` on the target object) and suppressing `BotFindBestTarget()` acquisition and HUNT pursuit unless a detection event has recently fired.
