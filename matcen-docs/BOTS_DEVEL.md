...
**Status:** Matcen 0.9.1-dev — Phase 7.6 non-convex room navigation: flow-field LOS gate (when the straight line to the next portal is wall-blocked, defer to the engine path-follower instead of beelining into glass/pillars), carrier sprint-orient (flag carrier in home room faces the home flag so afterburner fires), and outdoor flow-field disable (RF_EXTERNAL rooms use the engine path-follower — flow field routed bots into the sky barrier on canyon maps). Validated across pumphouse/MAYHEM/frenzy/bedlam/HAVOC CTF — escalations near-zero outside pumphouse (down from 530 collisions on pumphouse alone). Built on Phase 7.5, 7.4, 7.3, 7.2b, 7.2a, 7.2, 7.1.

Next milestone: 0.9.1 stable. Remaining known issues: cover-glass residual on pumphouse central room (bots in COMBAT/HUNT can press into an in-room glass barrier with LOS to a target — a wall-slide go-around was prototyped but the brake/slide path is gated on `!flow_dir` which is almost never true, so it never fired; needs the real `using_flow_field` flag threaded down before it can work). Occ-Dijkstra still uncached for rooms with 3+ portals (performance, not behavior). 18 blocked portals on abend2 (genuine geometry, not doors).
...
| 8.1a | **Outdoor steering — sky-flatten axis fix (0.9.1-dev):** First step of the Phase 8 outdoor navigation layer (design in `NAV_OVERHAUL_3.md`). `BotFlattenSkyDirection` flattened `dir.z()` as "world up", but **Y is the up axis** in this engine (`GetTerrainGroundPoint` writes `pos->y()`; the outdoor altitude caps use `pos.y()`). The original code operated on the horizontal depth axis — it never suppressed vertical motion and corrupted heading on strong +Z travel, which is why only fully disabling the flow field outdoors helped. Now correctly flattens `dir.y()`. Broader research found `AIF_BIASED_FLIGHT_HEIGHT` (the engine's terrain-relative altitude regulator) is gated on `ai_type == AIT_BIRD_FLOCK1` and never runs for our `AIG_GET_TO_POS` path-followers — so altitude regulation must be built in-fork (8.1b). New `$terrainsteer on\|off` toggle gates the layer. terrain1.log (CanyonsCTF): zero ceiling/sky spam, CTF objective nav working; remaining friction is base↔canyon boundary transitions. | Complete |
| 8.1e | **Outdoor steering — re-enable flow field in RF_TOUCHES_TERRAIN (0.9.1-dev):** Diagnostics (terrain2/3/4.log) proved CanyonsCTF and Bedlam's outdoor levels are 100% portal-connected `RF_TOUCHES_TERRAIN` rooms (no heightmap-cell occupancy), and that the Phase 7.6 blanket flow-field disable — not geometry — was stranding bots. With the 8.1a axis fix in, dropped `RF_TOUCHES_TERRAIN` from the disable at `bot_steering.cpp` (kept `RF_EXTERNAL`). `flow=1` returned on intra-area room hops (was 0/317 → 509/850 on CanyonsCTF); bots navigate within a canyon area. Still no captures alone — the open-sky crossing between areas was still bailing. | Complete |
| 8.1f | **Outdoor steering — terrain-region crossing heading + cross-commit latch (0.9.1-dev):** The flag round-trip requires crossing an open-sky terrain region (CanyonsCTF: region 36 on 53% of frames; Bedlam 67%). Instrumentation proved the engine `movement_dir` *oscillates* at the crossing mouth — 57% aligned with the sky-exit portal / 28% perpendicular-or-backward — so bots hovered and never crossed (`outside=0` on 100% of samples). `BotFlowFieldGetDirection` now generates its own stable heading toward the crossing mouth: exit-portal `path_pnt` when in-room with a terrain-region next-hop, entry-portal `path_pnt` when out on the terrain (`BOA_GetNextRoom` accepts terrain-cell roomnums). A per-bot latch (`bot_info.terrain_cross_*`, `BOT_TERRAIN_CROSS_REACH`/`TIMEOUT`) holds one mouth at a time to prevent threshold re-flapping; `BotFlattenSkyDirection` is skipped while a crossing is active so an overhead mouth isn't zeroed. See `NAV_OVERHAUL_3.md` §2.4.5. | Built, awaiting test |
| 8.1b+8.1d+2.2.5 | **Outdoor steering — mode decision + altitude band + ENTRANCE-SEEK (0.9.1-dev):** Ships the three-component unit per `NAV_OVERHAUL_3.md` §2.2.5. Per-frame mode decision: BOA next-hop is a terrain region → OPEN-TERRAIN mode (altitude band holds vertical 15-60u AGL); BOA next-hop is an indoor room → ENTRANCE-SEEK mode (band disabled, bot dives into entrance). Built after terrain6.log confirmed 8.1f alone causes ceiling-pin: the heading generator correctly aims at the sky-exit portal `path_pnt`, but without the altitude band, bots thrust up into the canyon ceiling. The band fixes this by suppressing upward thrust above 60u AGL and boosting it below 15u AGL. 8.1c (look-ahead climb via `ait_GetGroundInfo`) remains deferred. | Built, awaiting test |
