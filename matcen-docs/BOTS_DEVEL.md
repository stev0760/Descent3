...
**Status:** Matcen 0.8.6 — Per-bot team pre-assignment (`BotTeam<n>=` in `bots.cfg`, optional team arg to `$addbot`). All 11 team assignment test scenarios pass. Phase 5.5 complete. **Unreleased on branch:** cloak detection + hearing awareness (Phase 6.0 below). Prior (0.8.5): Fixed Plasma/EMD never being selected (`BotGetWbWeaponId()` fix), `$setpps` clamp raised to 2–40. See `BOT_MANAGEMENT.md` for design rationale.

Next release (0.9.0) will bundle cloak/hearing + chat commands + initial game-mode awareness.
...
- **Bots ignore player cloaking (FIXED, unreleased)** — Fixed in Phase 6.0: `BotCanSeeTarget()` skips cloaked targets in selection/firing/LOS unless a reveal condition applies (afterburner, headlight, napalm, or recent weapon fire). Bots also enabled as full participants in the engine's `AIN_HEAR_NOISE` pipeline (`hearing = 1.0f`). To be shipped in 0.9.0.
