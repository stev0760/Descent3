...
**Status:** Matcen **0.9.8 (released 2026-07-18, tag v0.9.8)** — the game-modes release: Entropy E1–E3 (first working bot takeovers), Monsterball M1–M2.6 (map-class scoring profile proven), CTF role structure (`$nav runner`), Hyper-Anarchy orb roles (`$nav hyper`), plus the release-session fixes (`$botstat` SIGSEGV, wind-aware Entropy targeting, ~90% log-footprint cut — row 6.17). Validated over the 07-13→18 hosted-server campaign (overnight soaks + live PiccuEngine play). Known-open, non-gating: Veins finisher conversion, Inversion refused-pickup spam, Rim nav-hostility, RAGE wind-fix re-test (instrumented). Prior stable: **0.9.7 (2026-07-12, tag v0.9.7)** — the single-spatial-authority navigation release (full history `NAVIGATION.md`, release view `CHANGELOG.md`).
