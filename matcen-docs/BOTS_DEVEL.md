...
**Status:** Matcen 0.9.1-dev — Phase 7.4 log-analysis fixes: powerup long-term blacklist (breaks Plasmacannon reselection loop) + occupancy-aware one-hop reroute (restores dispersal on blocked-portal code path). Built on Phase 7.3 outdoor fixes, 7.2b Dijkstra, 7.2a wall skating, 7.2 flow field, and 7.1 potential field.

Next milestone: 0.9.1 stable (second overnight test confirming Plasmacannon loop broken + dispersal working on reroute path).
